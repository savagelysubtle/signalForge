...
async def list_recommendations(
    ...
    if decision_ids:
        out_resp = (
            ...
            .select("id, recommendation_id, pnl_dollars, pnl_percent, exit_reason, logged_at")
            .eq("user_id", user_id)
            .in_("recommendation_id", rec_ids)
            .execute()
        )
    ...
    results: list[RecommendationWithStatus] = []
    for r in recs:
        dec = dec_map.get(r["id"])
        out = out_map.get(r["id"])
